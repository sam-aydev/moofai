...
import { translateMyText } from "@/supabase/server/action";
...
export async function TranslateAction({
  ...
  try {
    ...
    if (!response.ok) throw new Error("There was error in getting service");
    const result = await response.text();

    console.log(result);
    translateMyText({ inputedText, expectedLanguage, inputedLanguage, result });
  } catch (error) {
    console.error(error);
  }
}
